  ...
  (let [data (atom "")]
    (fn [email in-flight already-registered?]
      [:div.ui.form.attached.fluid.segment
       ...
       [(if-not (or @in-flight (not (re-find #".+@.+\..+" @data))) :button.ui.green.button :button.ui.green.button.disabled) {:disabled (or @in-flight (= (string/trim @data) "")) :on-click #(do (send-registration (string/trim @data) email in-flight already-registered?))} "Register"]])))
