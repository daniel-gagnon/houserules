    ...
    (fn []
      [:div#sign-in.ui.error.form
       ...
       [:div.ui.form.attached.fluid.segment
        ...
        [:input.ui.input {:placeholder "Email" :disabled @in-flight :auto-focus true :on-change #(do (reset! error false) (reset! email (string/trim (-> % .-target .-value)))) :on-key-down #(when (= 13 (.-keyCode %)) (login))}]
        [:input.ui.input {:placeholder "Password" :type :password :disabled @in-flight :on-change #(do (reset! error false) (reset! password (-> % .-target .-value))) :on-key-down #(when (= 13 (.-keyCode %)) (login))}]
